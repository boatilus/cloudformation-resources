...
import { <%= service.name %> } from "./<%= service.filename %>";
<% }) %>
...
for (const sp of servicePages) {
  ...
  try {
    ...
    if (rendered) {
      try {
        await Deno.mkdir(filenameRoot);
      } catch {
        // The folder already exists; no big deal.
      }

      await Deno.writeTextFile(path.join(filenameRoot, "index.ts"), rendered);
    }

    const testRendered = await eta.render(testTemplate, {
      filename: path.join(filenameRoot, "index.ts"),
      service: sp.service,
      names,
    });
...
const joined = services.map((service) => {
  return {
    name: service,
    filename: path.join(service.toLowerCase(), "index.ts"),
  };
});
...
if (indexRendered) {
  await Deno.writeTextFile("index.ts", indexRendered);
}
